...
namespace Nox.FFmpeg.Connectors {
	...
	public class AudioSourceConnector : BaseConnector {
		...
		private void Start() {
			...
			worker.OnPause.AddListener(Pause);
			worker.OnResume.AddListener(Resume);
			worker.OnSeek.AddListener(Seek);
			worker.OnVolumeChange.AddListener(SetVolume);
			worker.AddQueue += AddQueue;
		}

		private void OnDestroy() {
			worker.OnPause.RemoveListener(Pause);
			worker.OnResume.RemoveListener(Resume);
			worker.OnSeek.RemoveListener(Seek);
			worker.OnVolumeChange.RemoveListener(SetVolume);
			worker.AddQueue -= AddQueue;
		}
		...
		private void TryCreateNewClip(float[] pcm, int channels, int frequency, bool thread) {
			...
			if (clip == null || clipchannels != channels || clipfrequency != frequency) {
				maxEmptyReads = (int)(frequency * channels * worker.bufferSize);
				newClip       = true;
			}
			if (thread) {
				if (newClip) {
