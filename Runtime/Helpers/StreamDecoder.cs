...
namespace Nox.FFmpeg.Helpers {
	public sealed unsafe class StreamDecoder : IDisposable {
		private readonly BaseSource _ctx;
		...
		public StreamDecoder(BaseSource ctx, AVMediaType type, AVHWDeviceType HWDeviceType = AVHWDeviceType.AV_HWDEVICE_TYPE_NONE) {
			_ctx = ctx;
			AVCodec* codec = null;
			_streamIndex = ffmpeg
		...
		public bool CanDecode() {
			if (_ctx.IsEnded || _ctx._pPacket->stream_index != _streamIndex) {
				return false;
			}
			return true;
		}
